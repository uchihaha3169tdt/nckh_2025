...
def done_main():
    ...
    """
    import time
    import json  # Đã thêm thư viện json
    split = "test"
    ...
    for sample_i in range(args.num_samples):
        for rep_i in range(args.num_repetitions):
            ...
            # Đã thay đổi phần mở rộng file từ .mp4 sang .json
            save_file = sample_file_template.format(sample_i, rep_i).replace('.mp4', '.json')
            ...
            # ==========================================
            # PHẦN ĐÃ SỬA: LƯU KEYPOINTS RA FILE JSON
            # ==========================================
            num_frames = motion.shape[0]
            # Reshape mảng (T, 231) thành (T, 77 khớp, 3 tọa độ)
            motion_reshaped = np.reshape(motion, (num_frames, 77, 3))
            ...
            output_data = {
                ...
                "num_joints": 77,
                "keypoints": motion_reshaped.tolist()
            }

            with open(animation_save_path, 'w', encoding='utf-8') as f:
                json.dump(output_data, f, ensure_ascii=False)

            print(f"  [{caption} | Đã lưu keypoints tại: {animation_save_path}]")
